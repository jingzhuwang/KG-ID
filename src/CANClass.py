...
class FrameInfo:
    ...
    def _calculate_attribute(self):
        for key, item in list(self._fix_bits.items()):
            ...
            self._fix_bits[key][1] = int.from_bytes(tmp, byteorder='big', signed=False)  # 取值
    ...
    def _calculate_interval(self):
        ...
        if len(interval) == 0:  # only apper once
            ...
            # std = np.std(interval, ddof=1)
            if var < self._MAX_VARIANCE:
                self._interval = round(np.mean(interval), 6)
                # max_jitter = round(max(interval) - self._interval, 6)
                # min_jitter = round(min(interval) - self._interval, 6)
